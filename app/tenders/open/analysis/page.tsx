  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Link
              href="/tenders/open"
              className="inline-flex items-center text-xs font-semibold text-slate-600 hover:text-slate-900 transition-colors bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg border border-slate-200"
            >
            ...
            <div className="hidden sm:flex items-center gap-2 text-xs">
              ...
              <span className="text-blue-700 font-mono font-bold">{tender?.bidNumber || cleanBidNumber || targetId}</span>
            </div>
          </div>
            ...
            {analysis && (
              <>
                ...
                <Button
                  ...
                  className="bg-white hover:bg-slate-100 text-slate-800 border-slate-300 text-xs h-8 shadow-xs font-semibold"
                  title="Download Professional Minimal PDF Analysis"
                >
                  <Download className={`w-3.5 h-3.5 mr-1.5 text-blue-600 ${generatingPdf ? 'animate-bounce' : ''}`} />
                ...
                <Button
                  ...
                  className="bg-[#25D366] hover:bg-[#1EBE5D] text-white text-xs h-8 px-3 shadow-xs font-semibold flex items-center gap-1.5"
                ...
                <Button
                  ...
                  className="bg-white hover:bg-slate-100 text-slate-700 border-slate-300 text-xs h-8 shadow-xs"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        ...
        <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-blue-950 border border-slate-800 rounded-2xl p-6 shadow-lg mb-6 text-white">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-mono text-xs font-bold bg-blue-500/20 text-blue-300 border border-blue-400/30 px-3 py-1 rounded-full">
                ...
                {placeBadge && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-white font-bold text-xs">
            ...
            {tender?.pdfUrl && (
              <div className="flex items-center gap-2">
                <a
                  ...
                  className="inline-flex items-center text-xs font-semibold px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white border border-white/20 transition-colors shadow-sm"
                >
        ...
        {/* Loading Banner */}
        {loading && (
          <div className="bg-white border border-blue-200 rounded-2xl p-12 text-center shadow-sm my-8">
            <RefreshCw className="w-10 h-10 text-blue-600 animate-spin mx-auto mb-3" />
            <h3 className="text-base font-bold text-slate-900">AI Deep Document Intelligence in Progress...</h3>
        ...
        {/* Error Banner */}
        {error && !loading && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-red-800 text-sm mb-6 flex items-start gap-3">
        ...
        {analysis && !loading && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-xs">
              <span className="text-slate-500 font-medium text-xs block">EMD Amount (ईएमडी राशि)</span>
            ...
            <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-xs">
              <span className="text-slate-500 font-medium text-xs block">Estimated Bid Value</span>
            ...
            <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-xs">
              <span className="text-slate-500 font-medium text-xs block">Total Quantity</span>
            ...
            <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-xs">
              <span className="text-slate-500 font-medium text-xs block">ePBG / Security</span>
        ...
        {analysis && !loading && (
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            {/* Tabs Header */}
            <div className="border-b border-slate-200 bg-slate-50/80 px-4 sm:px-6 flex flex-wrap gap-2">
              <button
                type="button"
              ...
              >
                ...
                Items & Specifications ({analysis.items?.length || 0})
              </button>
              ...
              {activeTab === 'docs' && analysis.linkedDocuments && (
                <div className="space-y-3">
                  <p className="text-xs text-slate-500">
                    The following buyer-uploaded ATC & technical specification files were extracted from the tender PDF:
                  </p>
                  {analysis.linkedDocuments.map((doc, idx) => (
                    <div
                      key={idx}
                      className="bg-white border border-slate-200 hover:border-blue-400 p-4 rounded-xl flex items-center justify-between gap-4 transition-all shadow-xs"
                    >
                      <div className="flex items-center gap-3">
                        <FileText className="w-5 h-5 text-blue-600" />
                        <div>
                          <span className="text-sm font-bold text-slate-900 block">{doc.title}</span>
                          <span className="text-xs text-slate-500 font-mono line-clamp-1">{doc.url}</span>
                        </div>
                      </div>
                      <a
                        href={doc.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors flex-shrink-0"
                      >
